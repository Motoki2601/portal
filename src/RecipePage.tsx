...
export default function RecipePage({ user, onBack }: Props) {
  ...
  useEffect(() => subscribeRecipes(user.uid, setItems), [user]);

  const persist = (next: RecipeItem[]) => {
    setItems(next);
    saveRecipes(user.uid, next);
  };
  ...
  const handleToggleCooked = (id: string) => {
    const now = new Date().toISOString();
    persist(items.map(i => (i.id === id ? { ...i, cooked: !i.cooked, updatedAt: now } : i)));
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-white to-slate-50">
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-indigo-100/60 sticky top-0 z-40">
        <div className="max-w-2xl mx-auto px-5 py-4 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold tracking-tight text-indigo-900">✦ 作ってみたい料理</h1>
        </div>
      </header>
